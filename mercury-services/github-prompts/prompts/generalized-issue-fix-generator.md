---
name: generalized-issue-fix-generator
description: >
  GENERALIZED prompt generator for ISSUE-FIX / defect-resolution work on any module across the four
  standard workspaces (mercury-services, mercury-services-commons, appianway, mft-s3-aqua-appia). Given a
  small set of input arguments (module, output doc, goals, branch, Jira ticket, aws profile — mandatory;
  reference files, Confluence design link, ECS cluster, CloudWatch log group — optional), this prompt
  GENERATES a new, self-contained, module-specific issue-fix prompt under .github/prompts/issue-fix/ named
  <yyyymmdd>-<module>-<brief-summary>.md. The generated prompt carries the standard constraints, the
  session-context + token-usage telemetry protocol, the orient/back-up step, an optional AWS-log
  investigation step (read-only AWS CLI for the given profile, ECS cluster → service → task definition →
  CloudWatch logs; NO DynamoDB table scans), the root-cause analysis step, the failing-test-first
  reproduction step, the implementation step, the test + coverage step, and the verify/commit/document/handoff
  steps. The fix may itself involve refactoring. Generating a prompt MUST NOT modify this generator file.
argument-hint: "Fill the INPUTS block below, then run. Mandatory: module, doc, goals, branch, jira, aws_profile. Optional: references, confluence, ecs_cluster, cloudwatch_log_group."
agent: agent
...
# Module Issue-Fix — Prompt Generator

You are a **prompt generator**. When this file is run you do **not** perform the fix. Instead you read the
**INPUTS** block, validate it, and **write a new, self-contained module-specific issue-fix prompt** to
`.github/prompts/issue-fix/<yyyymmdd>-<module>-<brief-summary>.md`, by filling the **GENERATED-PROMPT TEMPLATE**
...
> *create* a new file under `.github/prompts/issue-fix/`. If that target file already exists, stop and ask the
> user whether to overwrite or pick a new brief-summary suffix.

---

## Standard workspaces (physical locations)

The generated prompt operates across these four repositories. Point the agent at the one that owns `module`
(the goals will make clear which); the others are read-only references unless the goals say otherwise.

| # | Workspace | Path | Role |
|---|-----------|------|------|
| 1 | `mercury-services` | `C:\Users\arijit.kundu\projects\mercury-services` | Main repo — all client-facing apps deployed in AWS. |
| 2 | `mercury-services-commons` | `C:\Users\arijit.kundu\projects\mercury-services-commons` | Shared library referenced by all mercury-services apps. Holds `cloud-sdk-api` + `cloud-sdk-aws` (AWS 2.x SDK), the `commons` utilities module, and the `dynamo-integration-test` DynamoDB-Local IT framework. **Other modules here are deprecated.** |
| 3 | `appianway` | `C:\Users\arijit.kundu\projects\appianway` | ETL repo — multiple modules for EDI file processing plus publisher/consumer modules that interact with watermill. |
| 4 | `mft-s3-aqua-appia` | `C:\Users\arijit.kundu\projects\mft-s3-aqua-appia` | Offshoot of `appianway` dealing with file processing. |

**Knowledge-base root:** `C:\Users\arijit.kundu\OneDrive - WiseTech Global\claude-workspace\_kbase`. Grep/search
it for the module in question, and check the module's own `docs/` folder.
...
references: |                   # one or MORE reference files / prior write-ups / design notes to read first
...
confluence: ""                  # Confluence design/analysis page URL
ecs_cluster: "ANEINVIS-002"                 # ECS cluster name — agent may inspect its service → task definition → logs (read-only)
cloudwatch_log_group: "inttra-ecs-logs"        # CloudWatch Logs log group to query (read-only) for error/root-cause evidence
```
...
1. **Validate inputs.** Confirm `module`, `doc`, `goals`, `branch`, `jira`, and `aws_profile` are all present and
   non-empty. If any mandatory input is missing, **stop and ask** the user — do not invent values. `references`
   may list one or more files. Optional inputs (`references`, `confluence`, `ecs_cluster`, `cloudwatch_log_group`)
   that are blank are simply omitted from the generated prompt (drop the corresponding section/bullet rather than
   emitting an empty one).

2. **Resolve derived values.**
   - `<yyyymmdd>` = today's date (UTC) in `YYYYMMDD` form.
   - `<module>` = the `module` input.
   - `<brief-summary>` = a 3–6 word kebab-case slug summarizing the issue/goals (e.g. `null-pointer-on-empty-edi`).
   ...
   - `<workspace>` = whichever of the four workspaces owns `module`. Resolve its physical path from the table above.
   - `references` may be a single file or many — render each as its own bullet.

   > **kb tooling note.** The mcp-context-server kb tools index the workspaces and `mercury-services-commons` (so
   > the agent can read cloud-sdk-api / cloud-sdk-aws sources directly), plus the knowledge-base root above.
   > `kb_grep` and `kb_find_files` are the reliable lookups; the semantic `kb_search` may return empty if its index
   > is not built — fall back to `kb_grep` / `kb_find_files`. The generated prompt tells the agent to use these. If not work, then log the gap and details in your documentation, you can also check the filesystem for the relevant workspaces directly if it helps
...
4. **Write the file** to `.github/prompts/issue-fix/<yyyymmdd>-<module>-<brief-summary>.md`. Do **not** modify this
...
# {{MODULE}} — Issue Fix — {{WORK_ID}}

> **Jira:** {{JIRA}}
> **Workspace:** {{WORKSPACE}} ({{WORKSPACE_PATH}})
> **AWS profile (read-only):** {{AWS_PROFILE}}
{{#CONFLUENCE}}
> **Confluence design/analysis:** {{CONFLUENCE}}
{{/CONFLUENCE}}
{{#ECS_CLUSTER}}
...
12. **AWS CLI IS READ-ONLY** — Any AWS CLI usage MUST use the `{{AWS_PROFILE}}` profile and be **READ queries only**
...
1. `session_list` — look for an existing session for `{{WORK_ID}}` / `{{MODULE}}` / this issue.
2. If found → `session_get` and **resume**; cross-link the prior session.
3. If none → `session_create`:
   - name: `{{MODULE}}-{{BRIEF_SUMMARY}}-{{YYYYMMDD}}`
   - project: `{{WORKSPACE}}`
   - tags: `["{{MODULE}}", "{{WORK_ID}}", "issue-fix", "{{BRIEF_SUMMARY}}", "in-progress"]`
...
The issue to diagnose and fix on the `{{MODULE}}` module — analyze it fully, find the root cause, and implement a
proper fix (which may involve refactoring), backward-compatible, all tests green with full coverage on new/changed
code, in exactly **one** unpushed commit referencing `{{WORK_ID}}`, with no design-doc references in code/commits:
...
exist**, or back it up if it does. Run these in the workspace that owns `{{MODULE}}` ({{WORKSPACE_PATH}}).

```bash
cd "{{WORKSPACE_PATH_BASH}}"
git fetch origin
...
git tag    {{WORK_ID}}-pre-fix-backup-{{YYYYMMDD}}
```

- Read the module and its `docs/` folder{{#REFERENCES}}, plus the reference material listed above (all reference
  files){{/REFERENCES}}. Use `kb_grep` / `kb_find_files` over the workspace, `mercury-services-commons`, and the kb
  root to locate the relevant code, prior analyses, and any cloud-sdk-api / cloud-sdk-aws implementations the fix
  touches.
- `jira_get_issue: {{JIRA}}` — read description / AC / comments for the reported behavior and constraints. Log findings.
{{#CONFLUENCE}}- Read the Confluence page ({{CONFLUENCE}}) for design/analysis context. Log findings.{{/CONFLUENCE}}
...
git log --oneline develop..HEAD            # exactly 1 line (or 0 if no fix committed yet), message contains {{WORK_ID}}
```
...
{{#AWS_INVESTIGATION}}
- **AWS logs (read-only, profile `{{AWS_PROFILE}}`)** — where the issue is a runtime/production symptom, inspect
...
{{#ECS_CLUSTER}}
  ...
  aws --profile {{AWS_PROFILE}} ecs list-services --cluster {{ECS_CLUSTER}}
  aws --profile {{AWS_PROFILE}} ecs describe-services --cluster {{ECS_CLUSTER}} --services <service>
  aws --profile {{AWS_PROFILE}} ecs describe-task-definition --task-definition <taskDefArn>   # find logConfiguration -> awslogs-group
  ```
{{/ECS_CLUSTER}}
{{#CLOUDWATCH_LOG_GROUP}}
  ```bash
  # CloudWatch Logs: search for the error/root-cause evidence (read-only)
  aws --profile {{AWS_PROFILE}} logs describe-log-streams --log-group-name "{{CLOUDWATCH_LOG_GROUP}}" \
    --order-by LastEventTime --descending --max-items 5
  aws --profile {{AWS_PROFILE}} logs filter-log-events --log-group-name "{{CLOUDWATCH_LOG_GROUP}}" \
    --filter-pattern "<error-or-correlation-id>" --start-time <epoch-ms> --end-time <epoch-ms> --max-items 100
  ```
{{/CLOUDWATCH_LOG_GROUP}}
{{^CLOUDWATCH_LOG_GROUP}}{{#ECS_CLUSTER}}
  From the task definition's `logConfiguration`, resolve the `awslogs-group`, then read it with
  `aws --profile {{AWS_PROFILE}} logs describe-log-streams` / `logs filter-log-events` (read-only).
{{/ECS_CLUSTER}}{{/CLOUDWATCH_LOG_GROUP}}
...
```bash
mvn -f {{MODULE}}/pom.xml test -Dtest=<NewReproTest>     # confirm it FAILS before the fix
```
...
```bash
mvn -f {{MODULE}}/pom.xml clean verify      # runs unit + integration tests and the coverage report
# Coverage report: {{MODULE}}/target/site/jacoco/index.html  (and per-submodule target/site/jacoco/*)
```
...
```bash
mvn -f {{MODULE}}/pom.xml clean verify     # unit + integration — BUILD SUCCESS, 0 failures
```
...
```bash
git add -A
git commit --amend -m "{{WORK_ID}}: {{MODULE}} — {{ONE_LINE_GOALS_SUMMARY}}"
...
**Document** all of today's work in `{{DOC}}` — the full analysis, root cause, and changes — including **every
search/grep/git/aws/analysis command with a clear descriptive comment** (Constraint 14) for reproducibility{{#REFERENCES}},
linking the reference docs{{/REFERENCES}}. Suggested structure:
...
2. **Investigation** — code-path trace, git-history findings, referenced cloud-sdk implementations reviewed, kb
   findings{{#AWS_INVESTIGATION}}, AWS-log evidence (read-only, profile `{{AWS_PROFILE}}`){{/AWS_INVESTIGATION}}.
...
10. **References** — {{JIRA}}{{#CONFLUENCE}}, {{CONFLUENCE}}{{/CONFLUENCE}}, reference files/docs.
...
- [ ] **(Coverage)** `mvn -f {{MODULE}}/pom.xml clean verify` BUILD SUCCESS; all new/changed code fully covered;
      relevant DynamoDB paths integration-tested.
- [ ] Every passing unit/integration test command logged in `{{DOC}}` with an accurate test-count summary.
- [ ] **(Constraint 12)** Any AWS CLI usage was read-only on the `{{AWS_PROFILE}}` profile; no DynamoDB scans.
...
- [ ] **(Constraint 14 / 15)** Every search/grep/git/aws/kb/analysis command logged with a descriptive comment in
      `{{DOC}}`; kb tooling (`kb_grep` / `kb_find_files`) used for lookups.
- [ ] `{{DOC}}` written with the full analysis, root cause, and changes.
...
| `{{DOC}}` | `doc` (mandatory) | output documentation file path (analysis + root cause + fix) |
| `{{GOALS_BULLETS}}` | `goals` (mandatory) | the issue/goals rendered as a bullet list (may be multi-line/paragraph) |
...
| `{{AWS_PROFILE}}` | `aws_profile` (mandatory) | named AWS CLI profile for read-only log/diagnostic queries |
| `{{WORKSPACE}}` | derived from `module` | which of the four workspaces owns the module |
| `{{WORKSPACE_PATH}}` | derived | the workspace's physical path (Windows form) |
| `{{WORKSPACE_PATH_BASH}}` | derived | the workspace path in Git-Bash form (e.g. `/c/Users/...`) |
...
| `{{ECS_CLUSTER}}` | `ecs_cluster` (optional) | ECS cluster; `{{#ECS_CLUSTER}}…{{/ECS_CLUSTER}}` dropped if blank |
| `{{CLOUDWATCH_LOG_GROUP}}` | `cloudwatch_log_group` (optional) | log group; conditional block dropped if blank |
...
Derived flag: `{{#AWS_INVESTIGATION}}…{{/AWS_INVESTIGATION}}` is included when **either** `ecs_cluster` **or**
`cloudwatch_log_group` is provided (the AWS-log investigation is meaningful only then); drop it if both are blank.

The `{{#X}}…{{/X}}` blocks are conditional: include them only when input `X` is provided (for `{{^X}}…{{/X}}`,
include only when `X` is **absent**). Only `references`, `confluence`, `ecs_cluster`, and `cloudwatch_log_group` are
optional. Never leave a raw `{{...}}` token or an empty heading in the generated file.
