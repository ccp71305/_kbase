---
name: confluence-runbook-generator
description: >
  Reusable prompt to generate an operational runbook and publish it to Confluence.
  Covers deployment procedures, rollback steps, monitoring, alerting, and incident response.
...
# Runbook Generator — Confluence Publisher (Reusable)
...
DESIGN_DOC_TEMPLATE: ".github/prompts/designdocs/templates/runbook-template.md"
```
...
2. Fetch Jira details for each ticket in `JIRA_TICKETS` (if applicable).
3. Generate the runbook following the template. Write to
   `<MODULE>/docs/<yyyy-mm-dd>-runbook-<topic>.md`.
